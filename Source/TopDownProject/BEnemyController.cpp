...
void ABEnemyController::Tick(float DeltaTime)
{
	...
	else if (CurrentState == DIE)
	{
		...
		case onUpdate:
		{
			...
			PawnAsNPC->DeathSequence(); // Plays possessed pawn's death sequence
			CurrentEvent = onExit;
		}
		break;
		case onExit:
...
{
	PawnAsNPC->SetAlert(false); // Sets possessed pawn's alert to false
}

states ABEnemyController::GetCurrentState()
{
	return CurrentState; // Returns CurrentState
}

UBlackboardComponent *ABEnemyController::GetBBoard()
{
	return BlackboardComp; // Returns BlackboardComp
}

UBehaviorTreeComponent *ABEnemyController::GetBTree()
{
	return BehaviorTreeComp; // Returns BehaviourTreeComp
}
